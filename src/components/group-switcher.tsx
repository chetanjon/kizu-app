  ...
  return (
    <div className="relative">
      <button onClick={() => setOpen((o) => !o)}
        className="flex items-center gap-2 font-m text-xs font-bold border-[2px] border-frame rounded-full px-3 py-1.5 bg-surface">
        <span className="w-2 h-2 rounded-full border-[1.5px] border-frame shrink-0" style={{ background: active?.color }} />
          ...
          <div className="absolute left-0 mt-2 w-[230px] z-40 bg-surface border-[2.5px] border-frame rounded-2xl shadow-[5px_5px_0_#0D0B09] overflow-hidden">
            ...
            {groups.map((g) => (
              <button key={g.id} onClick={() => switchTo(g.id)}
                ...
                <span className="w-2 h-2 rounded-full border-[1.5px] border-frame shrink-0" style={{ background: g.color }} />
                {g.name.toLowerCase()}
