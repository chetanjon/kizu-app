...
10. **Keep-warm:** a Supabase pg_cron job (`kizu-keep-warm`, every 5 min) pings TWO urls in one command: `https://kizu.app/` (keeps the **pages lambda** warm — this is the ping that prevents slow cold opens) and `https://kizu.app/api/cron/warm` (re-primes the 24h TMDB availability cache; deliberately unauthed — fixed bounded work, so no secret lives in the cron SQL). **The two are separate Vercel lambdas**: any route exporting its own `maxDuration`/`runtime` gets its own function, so pinging an API route does NOT warm the pages (2026-07-02 regression: cron pointed only at `/api/cron/warm` → 3s+ cold TTFBs on `/home` all day). As a backup, `/api/cron/warm` also self-pings `https://kizu.app/` — keep that fetch when editing the route. GitHub Actions cron proved too throttled for this (`keep-warm.yml` stays as a free backup layer). Don't remove the pg_cron job without a replacement; check it with `select jobid, command from cron.job`.
